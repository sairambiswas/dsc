...
import { Settings, MousePointer2, Type, Image as ImageIcon } from 'lucide-react';
...
const SpinningWheel: React.FC<SpinningWheelProps> = ({ 
  ...
  const spin = () => {
    ...
    setTimeout(() => {
      ...
      const safeIndex = Math.min(winningIndex, itemCount - 1);
      onResult(items[safeIndex]);
    }, spinDuration * 1000);
  };

  const handleWheelContextMenu = (e: React.MouseEvent) => {
    onContextMenu(e, [
      { label: 'Edit Slice Data', icon: <Settings size={16} />, onClick: () => setEditorTab('content') },
      { label: 'Typography Engine', icon: <Type size={16} />, onClick: () => setEditorTab('font') },
      { label: 'Hub Settings', icon: <ImageIcon size={16} />, onClick: () => setEditorTab('admin') },
    ]);
  };
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      <div 
        onContextMenu={handleWheelContextMenu}
        className="wheel-container relative w-80 h-80 md:w-[480px] md:h-[480px] cursor-context-menu"
      >
        {itemCount === 0 ? (
          ...
          <svg
            ref={wheelRef}
            viewBox="0 0 100 100"
            className="w-full h-full drop-shadow-[0_32px_80px_rgba(99,102,241,0.15)] overflow-visible"
          >
            <defs>
              <clipPath id="logoCircleClip"><circle cx="50" cy="50" r="9.5" /></clipPath>
            </defs>
            <g
              style={{ 
                transform: `rotate(${rotation}deg)`,
                transformOrigin: '50% 50%',
                transition: isSpinning ? `transform ${spinDuration}s cubic-bezier(0.15, 0, 0.15, 1)` : 'none'
              }}
            >
              {items.map((item, index) => {
                ...
                return (
                  <g key={item.id}>
                    ...
                    <text
                      x="82" y="50" fill="white" fontSize={fontSize} fontWeight="800" textAnchor="middle" dominantBaseline="middle"
                      className="pointer-events-none select-none" style={{ fontFamily: font }}
                      transform={`rotate(${startAngle + sliceSize / 2}, 50, 50)`}
                    >
              ...
              <circle cx="50" cy="50" r="11" fill="white" shadow-lg />
              {logoUrl ? (
                <image href={logoUrl} x="40.5" y="40.5" width="19" height="19" clipPath="url(#logoCircleClip)" preserveAspectRatio="xMidYMid slice" />
              ) : (
      ...
      <button
        ...
        style={{ backgroundColor: isSpinning ? '#f8fafc' : spinButtonColor, fontFamily: font, color: isSpinning ? '#94a3b8' : 'white' }}
        className={`mt-12 px-20 py-6 rounded-3xl font-black text-2xl shadow-2xl transition-all active:scale-95 group relative overflow-hidden
          ${isSpinning ? 'cursor-wait border border-slate-200' : 'hover:brightness-110 hover:-translate-y-1 shadow-indigo-100'}`}
      >
        <span className="relative z-10">{isSpinning ? 'CALCULATING VALOR...' : spinButtonText}</span>
        {!isSpinning && <div className="absolute inset-0 bg-white/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />}
      </button>
    </div>
  );
};
